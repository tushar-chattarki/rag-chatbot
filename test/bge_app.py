...
from reranking import ReRanking, BGE_ReRanker
...
# Header
st.title("Document Retrieval System using BGE ReRanker")
...
if submit_button and query:
    with st.spinner("Retrieving and reranking documents..."):
        try:
            ...
            # Rerank results
            reranked_chunks = BGE_ReRanker(query, results, 3)
            ...
            with tab1:
                ...
                if reranked_chunks:
                    for i, (node_data, score) in enumerate(reranked_chunks, 1):
                        with st.expander(f"Result {i} - Score: {score:.4f}", expanded=(i == 1)):

                            st.markdown(f"**Reranker Score:** `{score:.4f}`")

                            # Find the original node object from results
                            # node_data might be the node object or just content string
                            # We need to match it back to the original results
                            original_node = None
                            
                            # If node_data is a tuple with content
                            if isinstance(node_data, tuple):
                                content = node_data[1] if len(node_data) > 1 else str(node_data[0])
                            else:
                                content = str(node_data)
                            
                            # Find matching node from results
                            for res_node in results:
                                if res_node.get_content() == content or str(res_node.node) == content:
                                    original_node = res_node
                                    break
                            
                            # If we found the original node, check if it's an image
                            ...
                            # TEXT NODE
                                ...
                                # Show metadata if we found the original node
                                if original_node:
                                ...
                                st.text_area(
                                    ...
                                    content,
                                    height=250,
                                    key=f"reranked_txt_{i}_{hash(query)}",
                                    label_visibility="collapsed",
                                )
                else:
